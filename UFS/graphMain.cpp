...
// 684 medium 保证删除一条边，剩下的是一棵树。说明删除前有环。如果有多解，输出 edges 中靠后的那条。
// 从前往后遍历，遇到在同一个连通图的两个点，说明符合题意，因为保证只要删除一条边。
vector<int> findRedundantConnection(vector<vector<int>>& edges) {
    int n = edges.size();
    UFS ufs(n+1); // 是一棵树 + 一个节点，说明节点数为边数+1-1，又因为节点为 [1:n]，所以再+1
    for (const vector<int>& edge : edges) {
        int f1 = ufs.find(edge[0]), f2 = ufs.find(edge[1]);
        if (f1 == f2) {
            return edge;
        }
        ufs.unit(edge[0], edge[1]);
    }
    return {};
}
// 685 hard 684的有向图版本
// 1->2 2->3 3->1 这是有环无冲突
// 1->2 3->2 1->3 这是无环有冲突，指向2的节点有两个
// 1->2 2->3 3->1 4->2 有环有冲突
class Solution {
public:
    ...
    vector<int> findRedundantDirectedConnection(vector<vector<int>>& edges) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (fa[y] != y) { // y 已经有了父亲，所以 y 有两个父亲，其中一个边是冲突边
                conflictEdge = i;
            } else {
                fa[y] = x;
