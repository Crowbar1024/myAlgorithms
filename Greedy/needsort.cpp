...
// 621 medium 任务列表中的每个字母表示一种不同种类的任务。执行顺序任意，
// 每个任务执行时间为单位时间1。CPU一个单位时间可以完成一个任务，或者处于待命状态。
...
int leastInterval(vector<char>& tasks, int n) {
    ...
    for (char ch: tasks) ++taskNum[ch];
    int m = taskNum.size(), num = tasks.size(); // 任务种数和任务总数
    vector<int> nextTime(m, 1); // 一开始都可以执行
    vector<int> leftNum;
    ...
    while (num--) {
        ...
        for (int j = 0; j < m; ++j) {
            if (leftNum[j]) minNextValid = min(minNextValid, nextTime[j]);
        }
        cur = max(cur, minNextValid); // 跳到下一次可以执行的时间，避免等待
