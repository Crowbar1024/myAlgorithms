...
// 737 medium 两个句子是相似的条件：1 具有相同的长度。2 sentence1[i] 和 sentence2[i] 是相似的
...
bool areSentencesSimilarTwo(vector<string>& sentence1, vector<string>& sentence2, vector<vector<string>>& similarPairs) {
    ...
    for (vector<string>& similarPair : similarPairs) {
        ...
        if (strMap.count(s1) == 0) strMap[s1] = cnt++;
...
// 1061 medium s1==s2，说明s1[i]==s[2]，那么计算baseStr字典序排列最小的等效字符串
// 即把相等的字母放到一个集合中，然后按照字典序排序，然后baseStr每个字母转换成所在集合的最小字典序字母
string smallestEquivalentString(string s1, string s2, string baseStr) {
    UFS ufs(26);
    for (int i = 0; i < s1.size(); ++i) {
        ufs.unitSeq(s1[i] - 'a', s2[i] - 'a'); // 小的成为集合的头头
    }
    string ret(baseStr);
    for (char& ch : ret) {
        ch = ufs.find(ch - 'a') + 'a';
